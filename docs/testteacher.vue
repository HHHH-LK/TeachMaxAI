...
const currentUser = getCurrentUser()       // 之前已有
const myId = String(currentUser.id)  // 当前学生 id
// 使用师生对话组合式函数
...
function handleIncomingMessage(data) {
  // data 结构：{ senderUserId, receiverUserId, content, ... }
  if (String(data.receiverUserId) !== myId) return

  const fromId   = String(data.senderUserId)
  const msg = {
    sender: 'teacher',
  ...
  pushLocalMessage(fromId, msg)

  if (selectedTeacher.value?.id !== fromId) {
...
const handleSendMessage = async () => {
  ...
  if (!content || !selectedTeacher.value) return
  ...
  pushLocalMessage(teacherId, msg)
  newMessage.value = ''
  nextTick(scrollToBottom)

  /* 2. 再调后端 */
  try {
    await sendMessageToBackend(teacherId, content)
  } catch (e) {
...
/* 工具：把消息压入本地聊天记录 */
function pushLocalMessage(uid, msg) {
  if (!chatMessages.value[uid]) chatMessages.value[uid] = []
  chatMessages.value[uid].push(msg)
  if (selectedTeacher.value?.id === uid) {
    currentChatMessages.value = chatMessages.value[uid]
  }
}
