...
controller 负责对外提供 REST 接口与流式接口。
service / service.impl 承载教学、聊天、AI、游戏化等业务逻辑。
mapper + resources/mapper/*.xml 负责数据库访问。
model / handler / config 构成 AI Agent、RAG、工具调用、记忆管理等核心能力。
commons 提供统一返回、工具类、限流、延迟任务、异常处理等基础设施。
从仓库当前代码量看，核心后端规模大致为：
...
每个任务最终会交给对应 Handler 执行，例如增强型 RAG Handler、函数调用 Handler、普通聊天 Handler等。
...
带有游戏化学习机制的课程平台
License
当前仓库未看到明确的开源许可证配置。如果你计划公开到 GitHub，建议补充 LICENSE 文件并明确使用条款。
