...
const emit = defineEmits(['close', 'assign-teachers']);
...
const confirmAssignTeacher = () => {
  ...
  // 获取选中的教师对象列表
  const selectedTeachers = props.teachers.filter(t => selectedTeacherIds.value.includes(t.id));

  if (selectedTeachers.length > 0) {
    emit('assign-teachers', selectedTeachers);
    
    ElMessage({
      message: `已成功将${props.currentCourse.title}分配给${selectedTeachers.map(t => t.name).join('、')}`,
      type: 'success',
      duration: 2000
    });
