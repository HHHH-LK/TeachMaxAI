...
public class PromptUtils {
    ...
    public static String buildRagAnswerPrompt(String retrievedContent, String memoryContent, String userQuestion) {
        return String.format(
                "# 角色：专业RAG智能助手\n\n" +
                        ...
                        "2. 结合对话记忆保持上下文连贯\n" +
                        "3. 信息不足时可适当补充通用知识\n" +
                        "4. 完全无法回答时（（如写入、存放到桌面）、调用外部工具或执行 I/O），直接返回：\"%s\"",
    ...
    public static String buildSystemPrompt() {
        return """
           你是一个智能助手，具备使用工具获取外部信息的能力。当用户提出问题时，请按以下规则判断是否调用 SearchEngine 工具：
          
           1. 如果用户询问实时信息（如新闻、天气、日期等），或者问题中涉及模糊时间（例如“昨日”、“今天”）等，请先解析并明确具体的查询时间，再进行后续查询。
           2. 当需要获取最新数据、验证事实、或查询具体产品、服务及技术细节时，使用 SearchEngine 工具。
           3. 对于当前知识库无法回答或信息更新较快的问题，也应调用工具进行检索。
           在执行搜索前，请提取精确的关键词，避免冗长或不必要的查询语句，确保查询语句符合工具要求，从而提高搜索精度和效率。
           4.请严格避免输出任何不当、敏感、暴力或歧视性的内容。             
           """;
    }
    ...
    public static String buildCanPrompt(String memoryContent, String userQuestion) {
        return String.format(
                "# 任务：能力评估\n\n" +
                        ...
                        "### 判断标准：\n" +
                        "- ✅ 能够完整准确回答 → 回复：%s\n" +
                        "- ❌ 信息不足或不确定（如写入、存放到桌面）、调用外部工具或执行 I/O → 回复：%s\n\n" +
                        "**严格要求：仅输出判断结果，无需解释。**",
                formatSection("对话记忆", memoryContent),
                userQuestion,
                RESPONSE_CAN,
