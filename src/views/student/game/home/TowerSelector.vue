<template>
  <div class="tower-selector">
    ...
    <div class="tower-display" v-if="selectedTowerIndex > 0">
      ...
      <div
        class="debug-info"
        style="color: white; margin-bottom: 10px; font-size: 12px"
      >
        当前塔层数量: {{ towerLayers.length }} | 已解锁:
        {{ towerLayers.filter((l) => l.unlocked).length }} | 未解锁:
        {{ towerLayers.filter((l) => !l.unlocked).length }}
      </div>
            ...
            >
              ...
              <div class="tower-lock" v-if="!layer.unlocked">
                <svg viewBox="0 0 24 24" class="lock-icon">
                  <path
                    d="M12 1C8.676 1 6 3.676 6 7v2H5c-1.103 0-2 .897-2 2v9c0 1.103.897 2 2 2h14c1.103 0 2-.897 2-2v-9c0-1.103-.897-2-2-2h-1V7c0-3.324-2.676-6-6-6zm6 10.723V20H5v-8.277h1V7c0-2.761 2.239-5 5-5s5 2.239 5 5v3.723h2z"
                  />
                  <path
                    d="M12 12c-1.103 0-2 .897-2 2v3c0 1.103.897 2 2 2s2-.897 2-2v-3c0-1.103-.897-2-2-2z"
                  />
                </svg>
              </div>
...
export default {
  ...
  methods: {
    ...
    //点击层
    async handleLayerClick(layer) {
      // 检查是否已解锁
      if (layer.unlocked) {
        console.log(
          `跳转到测试页面，楼层ID: ${layer.floorId}, 塔ID: ${layer.towerId}`
        );
        ...
        try {
          ...
          // 获取当前选中的塔
          const currentTower = this.towers[this.selectedTowerIndex];
          const courseId = currentTower.courseId;

          console.log("调用 loadTest 接口参数:", {
          ...
          // 调用 loadTest 接口
          const response = await gameService.tower.loadTest(
            layer.towerId,
            layer.floorId,
            courseId,
          ...
          console.log("loadTest 响应:", response);

          if (response.data && response.data.success) {
            console.log("loadTest 调用成功:", response.data);
            ...
            const params = {
              ...
              towerLevel: layer.level,
            };

            console.log("准备跳转到测试页面，传递的参数:", params);
            ...
            console.error("loadTest 调用失败:", errorMessage);
            // 显示错误提示
            this.$toast.error(`加载测试失败: ${errorMessage}`);
          }
        } catch (error) {
          console.error("loadTest 调用异常:", error);
          // 显示错误提示
          this.$toast.error("加载测试失败，请稍后重试");
        } finally {
          // 隐藏加载状态
          this.loading = false;
        }
      } else {
        console.log(`第${layer.level}层尚未解锁`);
        this.$toast.warning(`第${layer.level}层尚未解锁，请先完成前一层任务`);
      }
    },
    // 渲染单个塔预览
      ...
      if (pickerTrack) {
        ...
        if (towerOption) {
          ...
          const scrollPosition =
            optionLeft - containerWidth / 2 + optionWidth / 2;
    ...
    async fetchTowerFloors(towerId) {
      try {
        ...
        const floorResponse = await gameService.tower.getTowerFloorByTowerId(
          towerId
        );
        ...
        if (
          floorResponse.data &&
          floorResponse.data.code === 0 &&
          floorResponse.data.data
        ) {
      ...
      try {
        ...
        const storyPromises = this.towers
          ...
          .map(async (tower) => {
            try {
              const storyResponse =
                await gameService.tower.getTowerStoryByTowerId(tower.towerId);
              console.log(`塔${tower.towerId}故事响应:`, storyResponse);

              if (
                storyResponse.data &&
                storyResponse.data.code === 0 &&
...
.tower-display {
  ...
  .nav-btn {
    ...
    &:hover {
      ...
      box-shadow: 0 0 40px rgba(255, 0, 0, 0.9),
        inset 0 0 25px rgba(255, 0, 0, 0.3), 0 8px 16px rgba(0, 0, 0, 0.8);
  ...
  .tower-part {
    img {
      width: 180px;
      height: auto;
      filter: drop-shadow(0 0 15px rgba(139, 0, 0, 0.9)) contrast(1.1)
        brightness(0.9);
      transition: all 0.3s ease;

      &:hover {
        filter: drop-shadow(0 0 25px rgba(255, 0, 0, 0.8)) contrast(1.2)
          brightness(1.1);
  ...
  .tower-layers {
    ...
    .tower-layer {
      ...
      img {
        width: 140px;
        height: auto;
        filter: drop-shadow(0 0 8px rgba(139, 0, 0, 0.7)) contrast(1.1)
          brightness(0.9);
        transition: all 0.3s ease;
      ...
      &.unlocked {
        ...
        img {
          filter: drop-shadow(0 0 15px rgba(0, 255, 0, 0.6)) contrast(1.2)
            brightness(1.1);
        }
      }
    }
