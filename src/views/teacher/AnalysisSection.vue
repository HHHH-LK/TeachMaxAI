<template>
  <div class="analysis-section">
    ...
    <div class="filter-bar">
      ...
      <button
        class="primary-btn"
        @click="emit('fetchAnalysisData')"
        :disabled="loading"
      >
        查询
      </button>
      <div style="flex: 1"></div>
      <button class="primary-btn ai-btn" @click="onAIClick" :disabled="loading">
        AI学情分析
      </button>
    </div>
            ...
            <div class="point-progress">
              <div
                ...
                :style="{
                  ...
                  backgroundColor: getColorByRate(point.masteryRate),
                }"
              ></div>
        ...
        <div class="card-header">
          ...
          <div class="exam-filter">
            ...
            <select
              ...
              @change="onExamChange"
              :disabled="
                !selectedCourse || examLoading || examList.length === 0
              "
            >
...
const loadTeacherInfoAndCourses = async () => {
  ...
  try {
    ...
    console.log("获取教师信息或课程列表失败:", error);
...
const masteryChartOption = computed(() => {
  return {
    ...
    legend: {
      orient: "vertical",
      left: "left",
      data: props.knowledgePoints.map((kp) => kp.name),
    },
    series: [
      {
        name: "掌握率",
        ...
        data: props.knowledgePoints.map((kp) => ({
    ...
    if (response.data && response.data.success) {
      ...
      for (const [pointId, masteryRate] of Object.entries(knowledgeData)) {
        try {
          ...
          console.log(`获取知识点${pointId}名称失败:`, error);
        }
      }
  ...
  try {
    ...
    if (response.data && response.data.success) {
      ...
      const filteredData = response.data.data.filter(
        (item) => item.wrongAnswerCount > 0
      );

      // 将过滤后的数据转换为组件需要的格式
      commonErrors.value = filteredData.map((item) => ({
        description: item.pointName,
        frequency: item.wrongAnswerCount,
        suggestion: `知识点: ${item.description}，难度: ${
          item.difficultyLevel
        }，关键词: ${item.keywords || "无"}`,
      }));

      //如果没有数据
      if (commonErrors.value.length === 0) {
        console.log("该课程没有高频错误知识点");
      }
    }
  } catch (error) {
    console.log("获取高频错误数据失败:", error);
...
const onCourseChange = async () => {
  ...
  if (selectedCourseId) {
    const selectedCourseObj = courses.value.find(
